                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  {item.customization?.color && (
                                    <div className="text-sm text-white/60">Couleur: {item.customization.color}</div>
                                  )}
                                  {item.customization?.size && (
                                    <div className="text-sm text-white/60">Taille: {item.customization.size}</div>
                                  )}
                                </div>
                              </div>
                            </TableCell>
                            ...
                            <TableCell>
                              {item.customization ? (
                                <Button 
                                  variant="outline" 
                                  size="sm"
                                  onClick={() => document.getElementById(`customization-modal-${index}`)?.showModal()}
                                >
                    ...
                    <div className="relative bg-white/5 rounded-lg p-2 aspect-square">
                      ...
                      {item.customization.designUrl && (
                        <div 
                          className="absolute"
                          style={{
                            top: `${item.customization.transform?.position.y || 50}%`,
                            left: `${item.customization.transform?.position.x || 50}%`,
                            transform: `translate(-50%, -50%) scale(${item.customization.transform?.scale || 1}) rotate(${item.customization.transform?.rotation || 0}deg)`,
                            width: '40%',
                            height: 'auto',
                          }}
                        >
                      ...
                      {item.customization.text?.content && (
                        <div 
                          className="absolute"
                          style={{
                            top: `${item.customization.text.transform?.position.y || 70}%`,
                            left: `${item.customization.text.transform?.position.x || 50}%`,
                            transform: `translate(-50%, -50%) scale(${item.customization.text.transform?.scale || 1}) rotate(${item.customization.text.transform?.rotation || 0}deg)`,
                            color: item.customization.text.color,
                            fontFamily: item.customization.text.font,
                            fontSize: '24px',
                            whiteSpace: 'nowrap',
                          }}
                        >
                    ...
                    <div>
                      ...
                      <div className="bg-black/40 p-4 rounded-lg space-y-2">
                        {item.design ? (
                          ...
                          <>
                            <p><strong>ID du design:</strong> {item.customization.designId}</p>
                            {item.customization.designName && (
                              <p><strong>Nom du design:</strong> {item.customization.designName}</p>
                            )}
                            <div className="mt-2">
                              <h5 className="text-sm font-medium mb-1">Aperçu du design:</h5>
                              <div className="w-20 h-20 rounded bg-white/10 overflow-hidden">
                    ...
                    <div>
                      ...
                      <div className="bg-black/40 p-4 rounded-lg space-y-2">
                        <p><strong>Position d'impression:</strong> {
                          item.customization.printPosition === 'front' ? 'Avant' : 'Arrière'
                        }</p>
                        <p><strong>Taille d'impression:</strong> {item.customization.printSize}</p>
                        
                        {item.customization.transform && (
                          <>
                            <p><strong>Position X:</strong> {item.customization.transform.position.x}%</p>
                            <p><strong>Position Y:</strong> {item.customization.transform.position.y}%</p>
                            <p><strong>Échelle:</strong> {item.customization.transform.scale}</p>
                            <p><strong>Rotation:</strong> {item.customization.transform.rotation}°</p>
                          </>
                        )}
                        ...
                        {item.customization.text && (
                          <>
                            ...
                            <p><strong>Contenu:</strong> {item.customization.text.content}</p>
                            <p><strong>Police:</strong> {item.customization.text.font}</p>
                            <p><strong>Couleur:</strong> <span className="inline-block w-4 h-4 rounded-full align-middle mr-1" style={{ backgroundColor: item.customization.text.color }}></span> {item.customization.text.color}</p>
                            <p><strong>Position:</strong> {
                              item.customization.text.printPosition === 'front' ? 'Avant' : 'Arrière'
                            }</p>
                            
                            {item.customization.text.transform && (
                              <>
                                <p><strong>Position X:</strong> {item.customization.text.transform.position.x}%</p>
                                <p><strong>Position Y:</strong> {item.customization.text.transform.position.y}%</p>
                                <p><strong>Échelle:</strong> {item.customization.text.transform.scale}</p>
                                <p><strong>Rotation:</strong> {item.customization.text.transform.rotation}°</p>
                              </>
                            )}
                          </>
                        )}
